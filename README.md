...
一张以地图视野查看中国、周边与海外城市当前及未来七天天气的天气地图。
...
- 天气覆盖范围切换：中国 / 中国 + 周边 / 海外；海外只展示天气，不提供自驾估算
- 当前、单日、7 日总览三种时间视图
- 七日无雨天数与最长连续无雨天数
- 城市搜索、键盘操作和按需定位
- 中国境内自驾估算：在城市详情中按需查询“我的位置 → 目的地”的高德驾车时间与距离；支持自动定位或手动输入中国城市，海外不调用路线服务
...
- 自驾估算使用 [高德 Web 服务 API](https://lbs.amap.com/api/webservice/guide/api/newroute) 的驾车路线，
  ...
  高德路线所需的国内坐标系，且不会持久化。手动输入的出发城市使用城市中心点，界面会明确标注
  “按城市中心估算”。
- 当前 GeoJSON 只用于产品原型。公开商业发布前必须完成地图边界、审图号与数据许可核验。
- 海外天气地图使用 [Natural Earth 1:110m](https://www.naturalearthdata.com/downloads/110m-cultural-vectors/)
  国家边界作为小比例尺底图；城市列表是 Sunward 维护的首批区域与全球城市中心点，后续可继续扩展。
